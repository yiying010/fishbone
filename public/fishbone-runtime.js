    ...
    function render(){if(typeof remotePaintPending!=="undefined"&&remotePaintPending&&!syncMuted){remotePaintPending=false;remoteDraftSnapshot=null}saveRoom();document.title="魚骨洞天";let teacher=$("teacherPanel"),preview=$("preview"),prevBtn=$("prevBtn"),nextBtn=$("nextBtn");$("stepNum").textContent=S.step===0?"起始頁":S.step+"/19";$("bar").style.width=(S.step===0?0:S.step/19*100)+"%";if(prevBtn)prevBtn.style.display=S.step<=1?"none":"";if(nextBtn)nextBtn.style.display=S.step>=19?"none":"";if(S.step===0){if(teacher)teacher.style.display="none";if(preview)preview.style.display="none";$("main").innerHTML=screen();return}if(teacher)teacher.style.display="";if(preview)preview.style.display="";$("aiTitle").textContent=stepNames[S.step-1];$("aiText").innerHTML=(ai[S.step]||[]).map(x=>`<p>${x}</p>`).join("");$("ttsToggle").textContent=S.autoTts?"關閉自動朗讀":"開啟自動朗讀";$("agentHint").innerHTML=agentFeedback();$("gateMsg").innerHTML="";$("main").innerHTML=screen();if(typeof applyHistoryReadOnly==="function")applyHistoryReadOnly();$("preview").innerHTML=collabPanel()+previewFish("right")+previewFish("left");if(!S.ttsReady)setTtsStatus("可按重播提示聆聽老師提示。");else if(S.autoTts&&S.lastSpokenStep!==S.step){S.lastSpokenStep=S.step;speakTeacher(false)}else setTtsStatus("")}
