    ...
    function render(){if(typeof remotePaintPending!=="undefined"&&remotePaintPending&&!syncMuted){remotePaintPending=false;remoteDraftSnapshot=null}saveRoom();document.title="魚骨洞天";let teacher=$("teacherPanel"),preview=$("preview"),prevBtn=$("prevBtn"),nextBtn=$("nextBtn");$("stepNum").textContent=S.step===0?"起始頁":S.step+"/19";$("bar").style.width=(S.step===0?0:S.step/19*100)+"%";if(prevBtn)prevBtn.style.display=S.step<=1?"none":"";if(nextBtn)nextBtn.style.display=S.step>=19?"none":"";if(S.step===0){if(teacher)teacher.style.display="none";if(preview)preview.style.display="none";$("main").innerHTML=screen();return}if(teacher)teacher.style.display="";if(preview)preview.style.display="";$("aiTitle").textContent=stepNames[S.step-1];$("aiText").innerHTML=(ai[S.step]||[]).map(x=>`<p>${x}</p>`).join("");$("ttsToggle").textContent=S.autoTts?"關閉自動朗讀":"開啟自動朗讀";$("agentHint").innerHTML=agentFeedback();$("gateMsg").innerHTML="";$("main").innerHTML=screen();$("preview").innerHTML=collabPanel()+previewFish("right")+previewFish("left");if(!S.ttsReady)setTtsStatus("可按重播提示聆聽老師提示。");else if(S.autoTts&&S.lastSpokenStep!==S.step){S.lastSpokenStep=S.step;speakTeacher(false)}else setTtsStatus("")}
    ...
    async function joinActivity(){
      ...
      /* Establish this tab's member identity before applying the authoritative
         server snapshot. Shared room data never comes from a browser cache. */
      ...
      if(outcome!=="ok"){
        ...
        render();
        showGate(outcome==="not-found"?"找不到這個房間碼，請再確認一次。房間碼由老師建立後提供。":outcome==="rate-limited"?"嘗試次數過多，請稍候一分鐘再試。":outcome==="room-full"?"這個小組已達設定人數並鎖定，請向老師確認房間碼。":outcome==="member-id-in-use"?"這個成員身分已由另一個工作階段使用，請回到原本的分頁重新加入。":"目前連不上小組伺服器，請確認網路後再試一次。");
      ...
      S.step=Math.max(2,S.step);S.reviewingStep=0;markInteraction();
      render();schedulePush();
    }
    ...
    async function createRoom(){
      ...
      let typedName=(($("joinName")||{}).value||"").trim(),expected=Number((($("expectedMemberCount")||{}).value||S.expectedMemberCount||0));if(typedName)S.nameDraft=typedName;
      if(!Number.isInteger(expected)||expected<1||expected>12){showGate("請設定 1 到 12 人的預計小組人數。");return}S.expectedMemberCount=expected;
      let btn=$("createRoomBtn");if(btn)btn.disabled=true;
      try{
        let res=await fetch(SYNC.base+"api/rooms",{method:"POST",cache:"no-store",headers:{"content-type":"application/json"},body:JSON.stringify({expectedMemberCount:expected})});
        if(res.status===429){showGate("建立房間的次數過多，請稍後再試。");return}
        if(!res.ok)throw new Error("HTTP "+res.status);
        let data=await res.json();
        S.roomCode=canonRoomCode(data.room||"");S.createdRoom=data.displayCode||showRoomCode(S.roomCode);applyRoomPolicy(data);
        render();
